...
export const RangeCommissionSlab = () => {
  ...
  const [modes] = useState(["GENERAL", "REAL"]);
  ...
  const fetchRules = async () => {
    setLoading(true);
    try {
      const response = await api.get("/admin/commission/range-rules", {
        params: {
          page,
          limit,
          search,
      ...
      console.error(error);
      toast.error("Failed to fetch range rules");
    } finally {
      setLoading(false);
    }
  };
  ...
  useEffect(() => {
    fetchRules();
  }, [page, search, slabFilter, operatorFilter, categoryFilter, roleFilter, statusFilter, modeFilter]);

  const handleCreate = async (e) => {
    e.preventDefault();
    try {
      const response = await api.post("/admin/commission/range-rules", {
        ...formData,
        amountFrom: parseFloat(formData.amountFrom || "0"),
        amountTo: parseFloat(formData.amountTo || "0"),
      ...
      if (response.data?.success) {
        toast.success("Range rule created in PENDING status!");
  ...
  const handleEdit = async (e) => {
    e.preventDefault();
    try {
      const response = await api.put(`/admin/commission/range-rules/${selectedRule.id}`, {
        ...formData,
        amountFrom: parseFloat(formData.amountFrom || "0"),
        amountTo: parseFloat(formData.amountTo || "0"),
        commissionValue: parseFloat(formData.commissionValue || "0"),
      ...
      if (response.data?.success) {
        toast.success("Range rule updated successfully!");
        setIsEditOpen(false);
        resetForm();
        fetchRules();
      }
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Are you sure you want to delete this range rule?")) return;
    try {
      const response = await api.delete(`/admin/commission/range-rules/${id}`);
      if (response.data?.success) {
  ...
  return (
    <div className="p-6 bg-[var(--bg-primary)] min-h-screen text-[var(--text-primary)]">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6 border-b border-[var(--border-soft)] pb-4">
        <div>
          <h1 className="text-xl font-bold tracking-tight uppercase">Range Commission Slab</h1>
      ...
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-soft)] p-3 rounded-lg mb-4 text-xs space-y-3">
        <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
          ...
          <div>
            <label className="block mb-1 font-bold text-[10px] text-[var(--text-secondary)] uppercase tracking-wider">Rule Mode</label>
            <select
              value={modeFilter}
      ...
      <div className="bg-[var(--bg-secondary)] border border-[var(--border-soft)] rounded-lg overflow-hidden relative shadow-xs">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-[var(--text-secondary)]">
            <span className="text-xs uppercase tracking-widest">No Range Commission Rules configured</span>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse text-[11px] select-none">
              <thead>
                <tr className="bg-[var(--bg-secondary)] border-b border-[var(--border-soft)] uppercase text-[9px] tracking-wider text-[var(--text-secondary)] font-bold sticky top-0">
                  ...
                  <th className="py-2.5 px-2 text-right">Amt From</th>
                  <th className="py-2.5 px-2 text-right">Amt To</th>
              ...
              <tbody className="divide-y divide-[var(--border-soft)] bg-[var(--bg-primary)]/10 font-medium">
                {rules.map((rule, index) => (
                  <tr key={rule.id} className="hover:bg-[var(--accent-hover)]/30 transition-colors">
                    ...
                    <td className="py-2 px-2 text-right font-mono font-bold">₹{rule.amountFrom}</td>
                    <td className="py-2 px-2 text-right font-mono font-bold">₹{rule.amountTo}</td>
                    ...
                    <td className="py-2 px-2 text-center font-bold text-[var(--text-secondary)]">{rule.mode}</td>
                    <td className="py-2 px-2 text-center">
                      {rule.status === "ACTIVE" ? (
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in text-xs font-semibold uppercase">
          <div className="bg-[var(--bg-secondary)] border border-[var(--border-soft)] w-full max-w-2xl rounded-lg shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="p-4 border-b border-[var(--border-soft)] flex justify-between items-center bg-[var(--bg-primary)]/10">
              <h2 className="font-bold text-sm tracking-tight">Create Range Rule</h2>
            ...
            <form onSubmit={handleCreate} className="p-4 space-y-4 overflow-y-auto flex-1">
              ...
              <div className="border-t border-[var(--border-soft)]/60 my-3 pt-3">
                <h3 className="font-bold text-[10px] text-[var(--text-secondary)] mb-2">AMOUNT RANGE & MODE</h3>
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block mb-1 text-[10px] text-[var(--text-secondary)]">Amount From (₹) *</label>
                    <input
                      required
                      type="number"
                      value={formData.amountFrom}
                      onChange={(e) => setFormData({ ...formData, amountFrom: e.target.value })}
                      className="w-full p-2 bg-[var(--bg-primary)] border border-[var(--border-soft)] rounded-md text-xs font-mono"
                    />
                  </div>
                  <div>
                    <label className="block mb-1 text-[10px] text-[var(--text-secondary)]">Amount To (₹) *</label>
                    <input
                      required
                      type="number"
                      value={formData.amountTo}
                      onChange={(e) => setFormData({ ...formData, amountTo: e.target.value })}
                      className="w-full p-2 bg-[var(--bg-primary)] border border-[var(--border-soft)] rounded-md text-xs font-mono"
                    />
                  </div>
                  <div>
                    <label className="block mb-1 text-[10px] text-[var(--text-secondary)]">Commission Mode *</label>
                    <select
                      value={formData.mode}
      ...
      {isEditOpen && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-fade-in text-xs font-semibold uppercase">
          <div className="bg-[var(--bg-secondary)] border border-[var(--border-soft)] w-full max-w-2xl rounded-lg shadow-xl overflow-hidden max-h-[90vh] flex flex-col">
            <div className="p-4 border-b border-[var(--border-soft)] flex justify-between items-center bg-[var(--bg-primary)]/10">
              <h2 className="font-bold text-sm tracking-tight">Edit Range Rule #{selectedRule?.id}</h2>
            ...
            <form onSubmit={handleEdit} className="p-4 space-y-4 overflow-y-auto flex-1">
              <div className="bg-[var(--color-primary-glow)]/20 border border-[var(--border-soft)] p-2.5 rounded-md mb-2 text-[10px]">
                Modifying target rule: Slab: <span className="font-bold text-[var(--color-primary)]">{selectedRule?.slab?.name}</span> | Operator: <span className="font-bold">{selectedRule?.operatorRel?.name}</span> | Role: <span className="font-bold">{selectedRule?.role}</span>.
              </div>

              <div className="border-b border-[var(--border-soft)]/60 pb-3">
                <h3 className="font-bold text-[10px] text-[var(--text-secondary)] mb-2">AMOUNT RANGE & MODE</h3>
                <div className="grid grid-cols-3 gap-3">
                  <div>
                    <label className="block mb-1 text-[10px] text-[var(--text-secondary)]">Amount From (₹) *</label>
          ...
          <div className="bg-[var(--bg-secondary)] border border-[var(--border-soft)] w-full max-w-md rounded-lg shadow-xl overflow-hidden">
            ...
            <form onSubmit={handleApprove} className="p-4 space-y-4">
              <div className="bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 p-2.5 rounded-md text-[10px]">
                Approving this range rule will set its status to <span className="font-bold">ACTIVE</span> and begin applying its amount-based rates to transactions.
              </div>
              <div>
          ...
          <div className="bg-[var(--bg-secondary)] border border-[var(--border-soft)] w-full max-w-md rounded-lg shadow-xl overflow-hidden">
            ...
            <form onSubmit={handleReject} className="p-4 space-y-4">
              <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 p-2.5 rounded-md text-[10px]">
                Rejecting this range rule will mark its status to <span className="font-bold">REJECTED</span>. It will remain in catalog logs but never activate.
              </div>
              <div>
